...
provider "yandex" {
  token     = var.yc_token #$(yc iam create-token)  
  cloud_id  = var.cloud_id #${yc config get cloud-id}
  folder_id = var.folder_id #${yc config get folder-id}
  zone = "ru-central1-d"
#"serviceAccount:${yandex_iam_service_account.k8s-sa.id}"
}

locals {
  zone_d_v4_cidr_blocks = "10.1.0.0/16" # Set the CIDR block for subnet in the ru-central1-d availability zone.
  folder_id             = var.folder_id            # Set your cloud folder ID.
  cloud_id		= var.cloud_id
  k8s_version           = var.k8s_version            # Set the Kubernetes version.
  sa_name               = "sa-terraform2"            # Set a service account name. It must be unique in a cloud.
}
